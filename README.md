...
| 非同期処理 | Kotlin Coroutines + Flow | 1.10.2 |
| データベース | Room | 2.8.4 |
| HTTP クライアント | Ktor Client | 3.4.0 |
| シリアライゼーション | Kotlinx Serialization | 1.10.0 |
| ナビゲーション | Compose Navigation (type-safe) | 2.9.7 |
| 画像読み込み | Coil | 3.3.0 |
| バックエンド | Firebase (Auth, Analytics, Crashlytics, Messaging, Remote Config) | BOM 34.9.0 |
| テスト | JUnit5 + Turbine + MockK | 5.14.3 / 1.2.1 / 1.14.9 |
| ビルド | Gradle (Kotlin DSL) + Version Catalog + Convention Plugins | 9.3.1 |
...
| ツール | バージョン | 備考 |
|--------|-----------|------|
| Android Studio | 最新安定版 | Ladybug 以降を推奨 |
| JDK | 21 | JVM ターゲットが JDK 21 に設定されている |
| Android SDK | API 36 (compileSdk / targetSdk) | SDK Manager でインストール |

[MUST] minSdk は **33** (Android 13) に設定されている。API 33 未満のデバイスはサポート対象外。
...
[WARNING] `google-services.json` が未配置の場合、ビルドは失敗する。
...
[MUST] コミット前に `spotlessCheck` と `detekt` がパスすることを確認する。
...
- Repository 実装は複数のデータソース (Network, Database, DataStore) を組み合わせる
...
| 項目 | 判断内容 |
|------|---------|
| Firebase 統合 | 抽象化と実装を分離 (`core/analytics`, `core/auth`, `core/notifications`) |
| Home Screen Widget | `AppWidgetProvider` でネイティブ実装。URI スキーム `poche://capture?type=memo\|photo\|voice` |
...
[MUST] ブランチプレフィックスは `feature/` (フルワード) を使用する。`feat/` は使用しない。
